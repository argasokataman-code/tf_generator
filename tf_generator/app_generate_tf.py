...
def load_accounts():
    if os.path.exists("auth_config.json"):
        try:
            ...
            st.error(f"Gagal load data akun: {str(e)}")
            return {"accounts": {}}
    return {"accounts": {}}
...
st.caption("Multi-Account Support | Simple Storage | Dynamic Bank Input")
        ...
        for idx, (window, transfers) in enumerate(window_groups.items()):
            with cols[idx % 3]:
                with st.expander(f"🪟 {window.upper()} ({len(transfers)} transfer)", expanded=True):
                    for t in transfers:
                        matched_accounts = []
                        if t['akun'] in accounts.get("accounts", {}):
                            matched_accounts = [
